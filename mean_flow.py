...
def imf_loss(
    ...
    """Improved MeanFlow velocity loss for the repository's noise-to-data path."""
    device = x_1.device
    batch_size = x_1.shape[0]
    x_0 = torch.randn_like(x_1)
    ...
    # The r=t boundary is an estimate of instantaneous velocity.  It is only a
    # JVP direction, not a learnable target path for this first-order loss.
    ...
    interval = (t - r).view(-1, 1, 1, 1)
    predicted_velocity = improved_velocity_from_mean_velocity(
